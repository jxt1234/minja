...
struct chat_template_inputs {
    rapidjson::Value messages; // Should be an array
    rapidjson::Value tools;    // Should be an array or null
    bool add_generation_prompt = true;
    rapidjson::Value extra_context; // Should be an object or null
...
class chat_template {
  ...
  private:
    ...
    // Helper to convert Value to pretty string
    static std::string valueToPrettyString(const rapidjson::Value& val) {
        rapidjson::StringBuffer buffer;
  ...
  public:
        ...
        : source_(source), bos_token_(bos_token), eos_token_(eos_token)
    {
        ...
        messages_for_render1.PushBack(dummy_str_user_msg, alloc);
        ...
        // Need to clone dummy_str_user_msg as try_raw_render might modify it (if polyfills were on)
        // For capability detection, polyfills are off, so copy is fine.
        rapidjson::Value dummy_str_user_msg_copy1; dummy_str_user_msg_copy1.CopyFrom(dummy_str_user_msg, alloc);
        ...
        rapidjson::Value dummy_typed_user_msg_copy1; dummy_typed_user_msg_copy1.CopyFrom(dummy_typed_user_msg, alloc);
        ...
        std::string out_tools_test = try_raw_render(messages_for_tools_test, tools_for_test, false, alloc);
        ...
        auto make_tool_call_rj = [&](const std::string& tool_name_str, rapidjson::Value& arguments_val, rapidjson::Document::AllocatorType& allocator_func) {
            ...
            func.AddMember("name", rapidjson::StringRef(tool_name_str.c_str()), allocator_func);
        ...
        tool_calls_array1.PushBack(make_tool_call_rj("ipython", tc1_args_str, alloc), alloc);
        ...
        if (caps_.supports_tool_calls) {
            ...
            rapidjson::Value dummy_args_for_parallel_test(alloc);
        ...
        if (!caps_.supports_tools) {
            ...
            rapidjson::Value arguments_infer(alloc);
            if (caps_.requires_object_arguments) {
    ...
    std::string apply(
        ...
        bool apply_polyfills = true)
    {
        fprintf(stderr, "[%s] Deprecated! Converting nlohmann to rapidjson for new apply().\n", __func__);
        
        Document d_deprecated_apply;
        auto& alloc = d_deprecated_apply.GetAllocator();

        chat_template_inputs inputs;
        inputs.allocator_for_inputs = &alloc; // Provide an allocator

        // Convert nlohmann::ordered_json to rapidjson::Value
        // This is a placeholder for proper conversion.
        // For complex structures, a recursive conversion function would be needed.
        // Assuming messages, tools, extra_context are simple enough or this deprecated function
        // will be removed soon.
        // A proper conversion would involve parsing the string dump of nlohmann::json.
        // e.g., inputs.messages.Parse(messages.dump().c_str()); - but this makes inputs.messages a Document.
        // We need inputs.messages to be a Value within the d_deprecated_apply document.

        // Simplified conversion: Parse from string dump.
        // This is inefficient but serves as a bridge for the deprecated function.
        std::string messages_str = messages.dump();
        rapidjson::Value messages_val(rapidjson::kArrayType);
        Document temp_doc_msg;
        if (!messages_str.empty() && temp_doc_msg.Parse(messages_str.c_str()).HasParseError()) {
             fprintf(stderr, "Error parsing messages in deprecated apply\n");
        } else if (!messages_str.empty()) {
            messages_val.CopyFrom(temp_doc_msg, alloc);
        }
        inputs.messages = messages_val;


        std::string tools_str = tools.dump();
        rapidjson::Value tools_val(rapidjson::kNullType); // Default to null if empty or parsing fails
        if (tools.is_array() && !tools.empty()) { // Only parse if it's a non-empty array
            Document temp_doc_tools;
            if (!temp_doc_tools.Parse(tools_str.c_str()).HasParseError()) {
                tools_val.CopyFrom(temp_doc_tools, alloc);
            } else {
                fprintf(stderr, "Error parsing tools in deprecated apply\n");
            }
        } else if (tools.is_array()) { // if it's an empty array
             tools_val.SetArray();
        }
        inputs.tools = tools_val;
        
        inputs.add_generation_prompt = add_generation_prompt;

        std::string extra_context_str = extra_context.dump();
        rapidjson::Value extra_context_val(rapidjson::kNullType);
        if (extra_context.is_object() && !extra_context.empty()) { // Only parse if it's a non-empty object
            Document temp_doc_extra;
            if (!temp_doc_extra.Parse(extra_context_str.c_str()).HasParseError()) {
                extra_context_val.CopyFrom(temp_doc_extra, alloc);
            } else {
                fprintf(stderr, "Error parsing extra_context in deprecated apply\n");
            }
        } else if (extra_context.is_object()){ // if it's an empty object
            extra_context_val.SetObject();
        }
        inputs.extra_context = extra_context_val;
    ...
    std::string apply(
        const chat_template_inputs & inputs_const, // const Value& makes CopyFrom necessary
        const chat_template_options & opts = chat_template_options()) const
    {
        ...
        // Make copies of inputs that can be modified
        chat_template_inputs inputs;
        inputs.allocator_for_inputs = &allocator; // Set allocator for the new inputs struct
        inputs.messages.CopyFrom(inputs_const.messages, allocator);
        inputs.tools.CopyFrom(inputs_const.tools, allocator);
        inputs.add_generation_prompt = inputs_const.add_generation_prompt;
        inputs.extra_context.CopyFrom(inputs_const.extra_context, allocator);
        inputs.now = inputs_const.now;
        ...
        std::string context_data_str = valueToString(context_data_val);
        nlohmann::json context_data_nlohmann = nlohmann::json::parse(context_data_str);
        context = minja::Context::make(context_data_nlohmann);
        ...
        if (opts.define_strftime_now) {
            ...
            context->set("strftime_now", MinjaValue::callable([time_now_capture](const std::shared_ptr<minja::Context> &, minja::ArgumentsValue & args) {
        ...
        if (!inputs.tools.IsNull()) {
            ...
            // Again, minja::Value constructor needs to handle rapidjson::Value
            // Temporary bridge: rapidjson::Value -> string -> nlohmann::json -> minja::Value
            std::string tools_for_minja_value_str = valueToString(inputs.tools);
            nlohmann::json tools_for_minja_value_nlohmann = nlohmann::json::parse(tools_for_minja_value_str);
            context->set("tools", MinjaValue(tools_for_minja_value_nlohmann));
        }
        if (!inputs.extra_context.IsNull() && inputs.extra_context.IsObject()) {
            for (auto & kv : inputs.extra_context.GetObject()) {
                ...
                // Temporary bridge for kv.value()
                std::string kv_value_str = valueToString(kv.value);
                nlohmann::json kv_value_nlohmann = nlohmann::json::parse(kv_value_str);
                context->set(kv.name.GetString(), MinjaValue(kv_value_nlohmann));
            }
        }

        auto ret = template_root_->render(context);
        // fprintf(stderr, "actual_messages: %s\n", valueToPrettyString(actual_messages).c_str());
    ...
    static rapidjson::Value add_system(
        ...
        rapidjson::Document::AllocatorType& allocator) // allocator for the returned Value
    {
